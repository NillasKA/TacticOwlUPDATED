...
namespace Tacticowl.DualWield
{
	...
	class Patch_PawnRenderer_DrawEquipmentAiming
	{
		...
		static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            return instructions.MethodReplacer(AccessTools.Method(typeof(PawnRenderer), nameof(PawnRenderUtility.DrawEquipmentAiming)),
		...
		public static void DrawBothEquipmentAiming(PawnRenderer __instance, Thing eq, Vector3 drawLoc, float aimAngle)
		{
			...
			{
				PawnRenderUtility.DrawCarriedThing(pawn, drawLoc, eq);
				return;
			}
